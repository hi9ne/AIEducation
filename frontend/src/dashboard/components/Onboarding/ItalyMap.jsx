...
const CITY_COORDINATES = {
  // Север Италии (Альпы и Паданская равнина)
  'Milano': { x: 120, y: 55 },        // Ломбардия, центр севера
  'Torino': { x: 90, y: 65 },         // Пьемонт, запад севера
  'Genova': { x: 105, y: 105 },       // Лигурия, на побережье
  'Venezia': { x: 175, y: 65 },       // Венето, северо-восток
  'Verona': { x: 155, y: 70 },        // Венето, между Миланом и Венецией
  'Padova': { x: 170, y: 70 },        // Венето, рядом с Венецией
  'Trento': { x: 150, y: 50 },        // Трентино, в Альпах
  'Trieste': { x: 195, y: 70 },       // Фриули, крайний северо-восток
  'Udine': { x: 185, y: 60 },         // Фриули, севернее Триеста
  'Brescia': { x: 140, y: 60 },       // Ломбардия, восток от Милана
  'Bergamo': { x: 130, y: 55 },       // Ломбардия, северо-восток от Милана
  'Como': { x: 120, y: 50 },          // Ломбардия, у озера Комо
  'Varese': { x: 115, y: 48 },        // Ломбардия, у швейцарской границы
  'Pavia': { x: 115, y: 65 },         // Ломбардия, юг от Милана
  'Cremona': { x: 135, y: 70 },       // Ломбардия, юго-восток от Милана
  'Mantova': { x: 150, y: 75 },       // Ломбардия, на границе с Венето
  
  // Центр-север (Эмилия-Романья)
  'Bologna': { x: 150, y: 85 },       // Столица Эмилии-Романьи
  'Parma': { x: 135, y: 80 },         // Эмилия-Романья, запад от Болоньи
  'Modena': { x: 145, y: 82 },        // Эмилия-Романья, между Пармой и Болоньей
  'Reggio Emilia': { x: 140, y: 81 }, // Эмилия-Романья
  'Ferrara': { x: 155, y: 78 },       // Эмилия-Романья, север от Болоньи
  'Ravenna': { x: 170, y: 85 },       // Эмилия-Романья, на Адриатике
  'Rimini': { x: 175, y: 95 },        // Эмилия-Романья, юг Адриатики
  
  // Центральная Италия (Тоскана, Умбрия, Марке)
  'Firenze': { x: 145, y: 115 },      // Столица Тосканы
  'Pisa': { x: 125, y: 118 },         // Тоскана, на Тирренском море
  'Siena': { x: 145, y: 125 },        // Тоскана, юг от Флоренции
  'Ancona': { x: 185, y: 120 },       // Марке, на Адриатике
  'Perugia': { x: 165, y: 130 },      // Умбрия
  'Terni': { x: 170, y: 140 },        // Умбрия, юг от Перуджи
  
  // Центр (Лацио, Абруццо)
  'Roma': { x: 170, y: 155 },         // Столица, в центре Лацио
  'Viterbo': { x: 160, y: 145 },      // Лацио, север от Рима
  'Frosinone': { x: 180, y: 165 },    // Лацио, юго-восток от Рима
  'Latina': { x: 175, y: 170 },       // Лацио, юг от Рима
  'Pescara': { x: 195, y: 135 },      // Абруццо, на Адриатике
  'Chieti': { x: 200, y: 140 },       // Абруццо
  'Teramo': { x: 190, y: 130 },       // Абруццо, север региона
  "L'Aquila": { x: 185, y: 135 },     // Абруццо, столица региона
  'Campobasso': { x: 205, y: 155 },   // Молизе
  'Isernia': { x: 200, y: 160 },       // Молизе
  
  // Юг Италии (Кампания, Пулья, Базиликата, Калабрия)
  'Napoli': { x: 190, y: 185 },       // Кампания, крупнейший город юга
  'Caserta': { x: 185, y: 180 },      // Кампания, север от Неаполя
  'Benevento': { x: 195, y: 180 },    // Кампания, внутренняя часть
  'Avellino': { x: 195, y: 190 },     // Кампания
  'Salerno': { x: 195, y: 200 },      // Кампания, юго-восток от Неаполя
  'Potenza': { x: 210, y: 210 },      // Базиликата, столица региона
  'Matera': { x: 220, y: 215 },       // Базиликата, знаменитые пещеры
  'Foggia': { x: 215, y: 175 },       // Пулья, север региона
  'Bari': { x: 230, y: 190 },         // Пулья, столица на Адриатике
  'Taranto': { x: 235, y: 210 },      // Пулья, на двух морях
  'Brindisi': { x: 240, y: 205 },     // Пулья, порт на Адриатике
  'Lecce': { x: 245, y: 220 },        // Пулья, крайний юго-восток
  'Cosenza': { x: 220, y: 235 },      // Калабрия, север региона
  'Catanzaro': { x: 230, y: 245 },    // Калабрия, столица
  'Reggio Calabria': { x: 220, y: 265 }, // Калабрия, крайний юг материка
  
  // Сицилия (крупнейший остров)
  'Palermo': { x: 170, y: 295 },      // Северо-запад Сицилии
  'Catania': { x: 200, y: 305 },      // Восток Сицилии, у Этны
  'Messina': { x: 210, y: 290 },      // Северо-восток, пролив
  'Siracusa': { x: 205, y: 315 },     // Юго-восток Сицилии
  'Agrigento': { x: 175, y: 310 },    // Юг Сицилии
  'Trapani': { x: 150, y: 300 },      // Крайний запад Сицилии
  'Caltanissetta': { x: 185, y: 305 }, // Центр Сицилии
  'Enna': { x: 190, y: 300 },         // Центр Сицилии
  'Ragusa': { x: 195, y: 320 },       // Юго-восток Сицилии
  
  // Сардиния (второй по величине остров)
  'Cagliari': { x: 115, y: 350 },     // Юг Сардинии, столица
  'Sassari': { x: 105, y: 320 },      // Север Сардинии
  'Nuoro': { x: 125, y: 335 },        // Центр Сардинии
  'Oristano': { x: 110, y: 345 },     // Запад Сардинии
  'Olbia': { x: 120, y: 315 },        // Северо-восток Сардинии
  'Carbonia': { x: 100, y: 355 },     // Юго-запад Сардинии
  'Iglesias': { x: 95, y: 350 }       // Юго-запад Сардинии
};
...
function getCityCoordinates(cityName) {
  ...
  // Затем проверяем частичное совпадение (нечувствительно к регистру)
  ...
  // Если не найдено, возвращаем координаты центра Италии
  console.warn(`Coordinates not found for city: ${cityName}`);
  return { x: 200, y: 180 };
}

const ItalyMap = ({ selectedCity, onSelectCity, universities: propUniversities, cities: propCities, isUniversityCardOpen = false }) => {
  const [cities, setCities] = useState(propCities || []);
  const [universities, setUniversities] = useState(propUniversities || []);
  ...
  const colors = {
    mapStroke: isDark ? '#2d3748' : '#718096',
    mapFill: isDark ? '#1a202c' : '#f7fafc',
    selectedCity: isDark ? '#ff6b6b' : '#e53e3e',
    defaultCity: isDark ? '#4299e1' : '#3182ce',
    cityHover: isDark ? '#63b3ed' : '#2b6cb0',
    cityText: isDark ? '#ffffff' : '#1a202c',
    cityLabel: isDark ? '#e2e8f0' : '#4a5568',
    tooltipBg: isDark ? '#1a202c' : '#ffffff',
    tooltipText: isDark ? '#ffffff' : '#1a202c'
  };

  // Обновляем внутреннее состояние при изменении пропсов
  useEffect(() => {
    if (propUniversities && propCities) {
    ...
    // Fallback - загружаем данные, если не переданы через пропсы
    const fetchUniversities = async () => {
      setLoading(true);
      try {
         console.log('ItalyMap: Fallback loading universities from real database...');
         ...
         console.log('ItalyMap: Fallback DB response:', data);
         ...
         // Фильтруем только по Италии
         const italianUnis = apiResults.filter(u => {
           const country = (u.country || '').replace(/\s+/g, '').toLowerCase();
           return country.includes('italy') || country.includes('италия');
         });
         
         console.log('ItalyMap: Fallback loaded universities from DB:', italianUnis.length);
    ...
    fetchUniversities();
  }, [propUniversities, propCities]);

  // Обработчики для клика по городу
            ...
            return (
              <g key={city}>
                ...
                <circle
                  cx={x}
                  cy={y}
                  r={Math.max(15, 12 + universityCount)}
                  fill="transparent"
                  stroke="none"
                ...
                {/* Основной маркер города */}
                <circle
                  cx={x}
                  cy={y}
                  r={Math.max(4, 3 + universityCount * 0.5)}
                  fill={isSelected ? colors.selectedCity : colors.defaultCity}
                  stroke="white"
                  strokeWidth="1.5"
                  className={`${styles.cityMarker} ${isSelected ? styles.selectedMarker : ''}`}
                  style={{ pointerEvents: 'none' }} // убираем pointer events с основного маркера
                />
                ...
                <text
                  x={x}
                  y={y + Math.max(8, 6 + universityCount * 0.5) + 8}
                  textAnchor="middle"
                  dominantBaseline="middle"
