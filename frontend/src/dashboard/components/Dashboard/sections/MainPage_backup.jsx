...
const ProgressSection = ({ user, deadlines = [], deadlinesLoading = false, deadlinesError = null, isMobile = false, isTablet = false }) => {
  ...
  // Theme aware design tokens (keep inline to avoid extra files). Adjust here for quick palette tuning.
  const colors = {
    // Layered backgrounds
    page: isDark ? '#0b1220' : '#f8fafc',
    surface: isDark ? '#0f172a' : theme.white,          // primary card
    surfaceAlt: isDark ? '#1f2937' : '#f1f5f9',         // secondary card / accent
    ...
    timeCardShadowTop: isDark ? 'inset 0 -8px 14px rgba(0,0,0,0.4)' : 'inset 0 -8px 14px rgba(0,0,0,0.05)',
    timeCardShadowBottom: isDark ? 'inset 0 8px 14px rgba(0,0,0,0.4)' : 'inset 0 8px 14px rgba(0,0,0,0.05)',
  ...
  const TimeCard = ({ value, label }) => {
    ...
    const topShade = {
      ...
      right: 0,
      height: '50%',
      boxShadow: colors.timeCardShadowTop
    };
    const bottomShade = {
      position: 'absolute',
      bottom: 0,
      ...
      right: 0,
      height: '50%',
      boxShadow: colors.timeCardShadowBottom
    };
    return (
      <Stack spacing={6} align="center" style={{ minWidth: 110 }}>
        <Box style={cardStyle}>
          ...
          <Text style={{ fontSize: 56, fontWeight: 800, lineHeight: 1, color: colors.timeCardNumber, letterSpacing: 1 }}>{str}</Text>
        </Box>
  ...
  const DigitalDisplay = ({ days, hours, minutes, seconds, isMobile = false, isTablet = false }) => {
    const fontSize = getResponsiveValue('20px', '32px', 'clamp(36px, 6vw, 72px)');
    const letterSpacing = getResponsiveValue(1, 1.5, 2);
    const padding = getResponsiveValue('0 4px', '0 6px', '0 8px');
        ...
        <Stack spacing="xs" align="center">
          <Group spacing="xs" align="center" wrap="nowrap">
            <Box style={{ textAlign: 'center' }}>
              ...
              <Text size="xs" style={{ color: colors.textSecondary }}>дни</Text>
            </Box>
            <Text style={colonStyle}>:</Text>
            <Box style={{ textAlign: 'center' }}>
              <Text style={commonStyle}>{pad(hours)}</Text>
              <Text size="xs" style={{ color: colors.textSecondary }}>часы</Text>
            </Box>
          </Group>
          <Group spacing="xs" align="center" wrap="nowrap">
            <Box style={{ textAlign: 'center' }}>
              <Text style={commonStyle}>{pad(minutes)}</Text>
              <Text size="xs" style={{ color: colors.textSecondary }}>мин</Text>
            </Box>
            <Text style={colonStyle}>:</Text>
            <Box style={{ textAlign: 'center' }}>
              <Text style={commonStyle}>{pad(seconds)}</Text>
              <Text size="xs" style={{ color: colors.textSecondary }}>сек</Text>
            </Box>
          </Group>
        </Stack>
      );
    ...
    return (
      <Group spacing={0} align="center" wrap="nowrap" style={{ justifyContent: 'center' }}>
        <Text style={commonStyle}>{String(days).padStart(2, '0')}</Text>
        <Text style={colonStyle}>:</Text>
        <Text style={commonStyle}>{pad(hours)}</Text>
        <Text style={colonStyle}>:</Text>
  ...
  return (
    <Stack spacing={getResponsiveValue('xs', 'sm', 'md')}>
      {/* Countdown card uses a slightly different layer in dark mode for contrast */}
      <Card 
        withBorder 
        p={getResponsiveValue('md', 'lg', 'lg')} 
        radius="md" 
        style={{ 
          background: isDark ? colors.surfaceAlt : colors.surfaceAlt, 
          transition: 'background 150ms ease' 
        }}
        className={isMobile ? 'mobile-card' : isTablet ? 'tablet-card' : ''}
      >
        <Stack spacing={getResponsiveValue(4, 6, 6)} align="center">
          <Text 
            size={getResponsiveValue('md', 'lg', 'xl')} 
            weight={800} 
            style={{ 
              letterSpacing: getResponsiveValue(0.8, 1.0, 1.2), 
              fontSize: getResponsiveValue(18, 24, 44), 
          ...
          >
            {isMobile ? 'До события осталось' : 'До следующего события осталось'}
          </Text>
          <DigitalDisplay 
          ...
          />
          {chosenEvent && (
            <Group spacing="xs" style={{ flexWrap: 'wrap', justifyContent: 'center' }}>
              <Badge 
                color={chosenEvent.type === 'note' ? 'grape' : 'red'} 
                variant="filled"
                size={getResponsiveValue('sm', 'md', 'md')}
              >
                {chosenEvent.type === 'note' ? 'Заметка' : 'Дедлайн'}
              </Badge>
              <Badge 
                color="gray" 
                variant="light"
                size={getResponsiveValue('sm', 'md', 'md')}
              >
                {toYMD(chosenEvent.date)}
              </Badge>
              <Text 
                size={getResponsiveValue('xs', 'sm', 'sm')} 
                style={{ 
      ...
      {/* Calendar and Progress - Responsive Layout */}
      <Stack spacing={getResponsiveValue('sm', 'md', 'lg')}>
        {/* Calendar Card */}
        <Card 
          withBorder 
          ...
          className={isMobile ? 'mobile-card' : isTablet ? 'tablet-card' : ''}
        >
            <Grid gutter={getResponsiveValue('xs', 'sm', 'md')}>
              <Grid.Col span={{ base: 12, md: isMobile ? 12 : 7 }}>
                  ...
                  renderDay={(date) => {
                    ...
                    return (
                      <div style={{ position: 'relative', width: '100%', height: '100%', opacity: isPast ? 0.5 : 1 }}>
                        <div style={{ 
                          textAlign: 'center', 
                          fontWeight: has ? 900 : 700, 
                          fontSize: getResponsiveValue(14, 16, 20), 
                          lineHeight: 1.2, 
                          color: colors.textPrimary 
                        }}>
                          {day}
              ...
              <Grid.Col span={{ base: 12, md: isMobile ? 12 : 5 }}>
                <Box>
                  ...
                  {deadlinesError && (
                    <Alert color="red" mb="md" size={getResponsiveValue('sm', 'md', 'md')}>
                      {deadlinesError}
                    </Alert>
                  )}
                    ...
                    <Stack spacing="xs">
                      {listedEvents.map((d, idx) => (
                        <Card 
                          key={d.id} 
                          withBorder 
                          p={getResponsiveValue('xs', 'sm', 'sm')} 
                          radius="md" 
                          style={{ 
                            background: colors.eventCardBg, 
                            border: colors.cardBorder, 
                            transition: 'background 150ms ease' 
                          }}
                          className={`event-card ${isMobile ? 'mobile-event-card touch-target' : ''}`}
                        >
                          <Group 
                            align="flex-start" 
                            spacing="sm" 
                            onClick={() => setFocusedEventId(d.id)} 
                            style={{ 
                              cursor: 'pointer',
                          ...
                          >
                            ...
                            <Badge 
                              size={getResponsiveValue('md', 'lg', 'lg')} 
                              color={d.type === 'note' ? 'grape' : 'red'} 
                              variant="filled" 
                              style={{ 
                                fontSize: getResponsiveValue(12, 14, 16), 
                                paddingInline: getResponsiveValue(8, 10, 12),
                            ...
                            <Box style={{ flex: 1 }}>
                              <Text 
                                size={getResponsiveValue('sm', 'md', 'lg')} 
                                weight={700} 
                                style={{ color: colors.eventCardTitle }}
                              >
                                {d.title}
                              </Text>
                              <Text 
                                size={getResponsiveValue('xs', 'xs', 'xs')} 
                                style={{ color: colors.eventCardDate }}
                              >
                                {toYMD(d.date)}
                              </Text>
                            </Box>
                            {d.type === 'note' && (
                              <Button
                                ...
                                variant="outline"
                                color="red"
                                className={isMobile ? 'touch-button' : ''}
                                styles={(theme) => ({
                                  root: {
                                    marginLeft: 'auto',
                                    marginRight: -6,
...
const MainPage = ({ isMobile = false, isTablet = false }) => {
  ...
  // Page-level colors (duplicated lightweight token set). If centralized palette appears later, move there.
  const pageColors = {
    page: isDark ? '#0b0f19' : '#f1f5f9',
    card: isDark ? '#162132' : '#ffffff',
    cardAlt: isDark ? '#1f2d40' : '#f8fafc',
    text: isDark ? theme.white : '#1f2937',
    textSecondary: isDark ? '#cbd5e1' : '#475569'
  };
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        try {
          ...
          setDeadlinesError(err.message || 'Ошибка загрузки событий');
        } finally {
